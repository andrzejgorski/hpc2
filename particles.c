...
void update_acceleration(particle *p1, particle *p2, particle *p3) {
    ...
    p1->new_acc_x -= 2 * (potential_x1 - potential_x2) / (up_x - down_x);
    ...
    p1->y_c = old_y;
    p1->new_acc_y -= 2 * (potential_y1 - potential_y2) / (up_y - down_y);
    ...
    p1->z_c = old_z;
    p1->new_acc_z -= 2 * (potential_z1 - potential_z2) / (up_z - down_z);
}
